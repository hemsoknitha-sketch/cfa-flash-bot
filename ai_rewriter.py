...
class SuperBrainAIRewriter:
    ...
    def _rule_based_fallback(self, raw_id: str, title: str, content: str, source: str, source_tier: int, is_unverified: bool) -> ProcessedNewsArticle:
        ...
        # Strip view/reaction count metrics e.g. "ចំនួនមើល 30 ពាន់ · ប្រតិកម្ម 3.1 ពាន់ | "
        clean_desc = re.sub(r'ចំនួនមើល[^\|]*?\|', '', clean_desc)
        clean_desc = re.sub(r'ប្រតិកម្ម[^\|]*?\|', '', clean_desc)
        clean_desc = re.sub(r'^\s*[\d\.\,]+\s*(?:ពាន់|លាន|K|M|views|reactions)\s*·?\s*', '', clean_desc, flags=re.IGNORECASE)
        clean_desc = re.sub(r'\s*\|\s*(?:Facebook|ពលរដ្ឋសកម្ម)$', '', clean_desc, flags=re.IGNORECASE).strip()
        ...
        # Smart Lead Story Generator for Short / Reel / Video Titles
        if not clean_desc or len(clean_desc) < 40 or clean_desc == clean_title:
            if any(k in clean_title for k in ["វិនិយោគ", "វិនិយោគិន", "សេដ្ឋកិច្ច", "ពាណិជ្ជកម្ម", "វិនិយោគិនល្អ"]):
                clean_desc = (
                    f"ការទាក់ទាញវិនិយោគិនផ្ទាល់ពីបរទេស (FDI) និងការបង្កើតបរិយាកាសវិនិយោគអំណោយផល គឺជាកាតាលីករយ៉ាងសំខាន់ក្នុងការរុញច្រានកំណើនសេដ្ឋកិច្ចជាតិកម្ពុជា។ "
                    f"ការលើកកម្ពស់តម្លាភាព នីតិរដ្ឋ និងការកាត់បន្ថយនីតិវិធីរដ្ឋបាល គឺជាកត្តាស្នូលក្នុងការកសាងជំនឿទុកចិត្តរបស់វិនិយោគិនអន្តរជាតិជាន់ខ្ពស់ មកបណ្តាក់ទុនរយៈពេលវែងនៅកម្ពុជា។"
                )
            elif any(k in clean_title for k in ["តុលាការ", "ក្រសួងយុត្តិធម៌", "ចៅក្រម", "ព្រះរាជអាជ្ញា", "យុត្តិធម៌"]):
                clean_desc = (
                    f"ការបែងចែកអំណាចយ៉ាងច្បាស់លាស់រវាងអំណាចតុលាការឯករាជ្យ និងក្រសួងយុត្តិធម៌ គឺជាគ្រឹះយ៉ាងសំខាន់ក្នុងការកសាងប្រព័ន្ធយុត្តិធម៌អព្យាក្រឹត្យ និងមានទំនុកចិត្តខ្ពស់។ "
                    f"ការគោរពព្រំដែនសមត្ថកិច្ច និងការកែទម្រង់ប្រព័ន្ធយុត្តិធម៌ ធានាថាសិទ្ធិ និងសេរីភាពរបស់ប្រជាពលរដ្ឋត្រូវបានការពារយ៉ាងម៉ឺងម៉ាត់បំផុតចំពោះមុខច្បាប់។"
                )
            elif any(k in clean_title for k in ["ឆបោក", "អនឡាញ", "បទល្មើស"]):
                clean_desc = (
                ...
                clean_desc = (
                    f"កិច្ចខិតខំប្រឹងប្រែងក្នុងការលើកកម្ពស់តម្លាភាព អភិបាលកិច្ចល្អ និងនីតិរដ្ឋ គឺជាកាតព្វកិច្ចចម្បងក្នុងការអភិវឌ្ឍសង្គមជាតិ។ "
                    f"ការចូលរួមយ៉ាងសកម្មរបស់ប្រជាពលរដ្ឋ និងស្ថាប័នពាក់ព័ន្ធ នឹងរួមចំណែកកសាងសង្គមមួយដែលមានស្ថិរភាព និងភាពយុត្តិធម៌ជានិរន្តរ៍។"
                )
        ...
        p2 = f"យោងតាមប្រភពព័ត៌មានផ្លូវការពី {source_name} បានបញ្ជាក់ឱ្យដឹងថា ព្រឹត្តិការណ៍ និងការចូលរួមយ៉ាងសកម្មនេះ គឺជាជំហានដ៏សំខាន់ក្នុងការលើកកម្ពស់តម្លាភាព គណនេយ្យភាពសង្គម និងការទប់ស្កាត់រាល់បាតុភាពអសកម្ម។"
